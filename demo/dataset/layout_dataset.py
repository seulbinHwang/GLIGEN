...
class LayoutDataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 random_flip=True):
        super().__init__(
            random_crop=None, random_flip=None, image_size=None
        )  # we only use vis_getitem func in BaseDataset, donot use the others.
        ...
        for category_data in stuff_data['categories']:
            ...
            self.object_idx_to_name[category_data['id']] = category_data['name']
        self.all_categories = [
            self.object_idx_to_name.get(k, None) for k in range(183 + 1)
        ]
        ...
        all_filenames = [
            self.image_id_to_filename[idx] for idx in self.image_ids
        ]
    ...
    def __getitem__(self, index):
        ...
        positive_embeddings = torch.zeros(self.max_boxes_per_image,
                                          self.embedding_len)
