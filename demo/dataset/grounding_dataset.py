...
class GroundingDataset(BaseDataset):
    ...
    def __init__(
        ...
        with open(json_path, 'r') as f:
            json_raw = json.load(
                f
            )  # keys: 'info', 'images', 'licenses', 'categories', 'annotations'
        self.data = json_raw[
            "images"]  # donot name it images, which is misleading
        ...
        self.data = {
            datum['data_id']: datum for datum in self.data
        }  # map self.data from a list into a dict
        ...
        self.file_name_to_data_ids = defaultdict(
            list)  # for each image, there are multiple data points (captions)
...
# check_filenames_in_zipdata(all_filenames, image_root)
    ...
    def __getitem__(self, index):
        ...
        for anno in annos:
            ...
            if valid:
                ...
                all_positive_embeddings.append(
                    torch.load(os.path.join(self.annotation_embedding_path,
                                            str(anno["id"])),
                               map_location='cpu'))
        ...
        positive_embeddings = torch.zeros(self.max_boxes_per_data,
                                          self.embedding_len)
