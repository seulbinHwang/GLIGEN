...
def clean_annotations(annotations):
    for anno_info in annotations:
        anno_info.pop("iscrowd",
                      None)  # I have checked that all 0 for flickr, vg, coco
        anno_info.pop(
            "category_id", None
...
def make_a_sentence(obj_names, clean=False):
    ...
    if clean:
        obj_names = [
            name[:-6] if ("-other" in name) else name for name in obj_names
        ]
    ...
    if random_drop_embedding == 'both':
        ...
        for i in range(N):
            if random.uniform(0, 1) < 0.5:  # else keep both features
                idx = random.sample(
                    [0, 1],
                    1)[0]  # randomly choose to drop image or text feature
                temp_mask[idx, i] = 0
...
class TSVDataset(BaseDataset):
    ...
    def __init__(
        ...
        assert which_layer in [['after', 'after'], ['before', 'after_renorm'],
                               ['before', 'after_reproject']]
    ...
    def mapping(self, image_embedding):
        if self.which_layer_image == 'after':
            ...
            image_embedding = project(image_embedding.unsqueeze(0),
    ...
    def __getitem__(self, index):
        ...
        is_det = raw_item.get(
            'is_det', False
        )  # if it is from detection (such as o365), then we will make a caption
        ...
        for anno in annos:
            ...
            if valid:
                ...
                all_boxes.append(
                    torch.tensor([x0, y0, x1, y1]) /
                    self.image_size)  # scale to 0-1
                all_masks.append(1)
                all_text_embeddings.append(anno[text_embedding_name])
                all_image_embeddings.append(
                    self.mapping(anno[image_embedding_name]))
                if is_det:
        ...
        text_embeddings = torch.zeros(self.max_boxes_per_data,
                                      self.embedding_len)
        image_embeddings = torch.zeros(self.max_boxes_per_data,
                                       self.embedding_len)
        if is_det:
            category_names = []
        ...
        if self.random_drop_embedding != 'none':
            image_masks, text_masks = mask_for_random_drop_text_or_image_feature(
                masks, self.random_drop_embedding)
        else:
            image_masks = masks
            text_masks = masks
