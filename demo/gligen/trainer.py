...
def create_expt_folder_with_auto_resuming(OUTPUT_ROOT, name):
    ...
    if os.path.exists(name):
        ...
        for previous_tag in all_existing_tags:
            potential_ckpt = os.path.join(name, previous_tag,
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.name, self.writer, checkpoint = create_expt_folder_with_auto_resuming(
            config.OUTPUT_ROOT, config.name)
        if get_rank() == 0:
            shutil.copyfile(config.yaml_file,
                            os.path.join(self.name, "train_config_file.yaml"))
        ...
        self.autoencoder = instantiate_from_config(config.autoencoder).to(
            self.device)
        self.text_encoder = instantiate_from_config(config.text_encoder).to(
            self.device)
        self.diffusion = instantiate_from_config(config.diffusion).to(
            self.device)

        state_dict = read_official_ckpt(
            os.path.join(config.DATA_ROOT, config.official_ckpt_name))
        ...
        if self.config.whole:
            ...
            assert set(all_params_name) == set(trainable_names +
        ...
        self.master_params = list(
            self.model.parameters()
        )  # note: you cannot assign above params as master_params since that is only trainable one
        ...
        dataset_train = ConCatDataset(config.train_dataset_names,
                                      config.DATA_ROOT,
                                      config.which_embedder,
                                      train=True,
                                      repeats=train_dataset_repeats)
        sampler = DistributedSampler(
            dataset_train) if config.distributed else None
        ...
        # = = = = = misc = = = = = #
        if get_rank() == 0:
            print("Actual total need see images is: ",
                  config.total_iters * config.total_batch_size)
            print("Equivalent training epoch is: ",
                  (config.total_iters * config.total_batch_size) /
                  len(dataset_train))
    ...
    def run_one_step(self, batch):
        ...
        loss = torch.nn.functional.mse_loss(model_output,
                                            noise) * self.l_simple_weight
    ...
    def start_training(self):
        ...
        iterator = tqdm(range(self.starting_iter, self.config.total_iters),
        ...
        for iter_idx in iterator:  # note: iter_idx is not from 0 if resume training
            ...
            if self.config.use_o2:
                ...
                with torch.cuda.amp.autocast(
                        enabled=enabled):  # with torch.autocast(enabled=True):
                    loss = self.run_one_step(batch)
            ...
            if self.config.enable_ema:
                update_ema(self.ema_params, self.master_params,
            ...
            if (get_rank() == 0):
                if (iter_idx % 10 == 0):
                    self.log_loss()
                if (iter_idx
                        == 0) or (iter_idx % self.config.save_every_iters
                                  == 0) or (iter_idx
                                            == self.config.total_iters - 1):
    ...
    def log_loss(self):
        for k, v in self.loss_dict.items():
            self.writer.add_scalar(k, v, self.iter_idx +
                                   1)  # we add 1 as the actual name
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            for i in range(batch_here):
                ...
                im = self.dataset_train.datasets[0].vis_getitem_data(
                    out=temp_data, return_tensor=True, print_caption=False)
                real_images_with_box_drawing.append(im)
            real_images_with_box_drawing = torch.stack(
                real_images_with_box_drawing)
            ...
            shape = (batch_here, model_wo_wrapper.in_channels,
                     model_wo_wrapper.image_size, model_wo_wrapper.image_size)
            input = dict(x=None,
                         timesteps=None,
                         context=context,
                         boxes=batch['boxes'],
                         ...
                         text_embeddings=batch["text_embeddings"],
                         image_embeddings=batch["image_embeddings"])
            samples = ddim_sampler.sample(S=50,
                                          shape=shape,
                                          input=input,
                                          uc=uc,
                                          guidance_scale=5)
            ...
            self.image_caption_saver(samples, real_images_with_box_drawing,
                                     batch["caption"], iter_name)
        ...
        torch.save(
            ...
            os.path.join(self.name,
                         "checkpoint_" + str(iter_name).zfill(8) + ".pth"))
