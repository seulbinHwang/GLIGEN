...
def draw_box(img, locations):
    colors = [
        "red", "green", "blue", "olive", "orange", "brown", "cyan", "purple"
    ]
    draw = ImageDraw.Draw(img)
    WW, HH = img.size
    for bid, box in enumerate(locations):
        draw.rectangle([box[0] * WW, box[1] * HH, box[2] * WW, box[3] * HH],
...
def load_ckpt(config, state_dict):
    ...
    text_encoder = instantiate_from_config(
        config.text_encoder).to(device).eval()
...
def get_clip_feature(model, processor, input, is_image=False):
    ...
    if is_image:
        ...
        inputs['pixel_values'] = inputs['pixel_values'].to(
            device)  # we use our own preprocessing without center_crop
        inputs['input_ids'] = torch.tensor([[0, 1, 2,
                                             3]]).to(device)  # placeholder
        ...
        if feature_type[1] == 'after_reproject':
            feature = project(
                feature,
                torch.load('gligen/projection_matrix').to(device).T).squeeze(0)
        ...
        inputs['pixel_values'] = torch.ones(1, 3, 224,
                                            224).to(device)  # placeholder
        inputs['attention_mask'] = inputs['attention_mask'].to(device)
        outputs = model(**inputs)
        feature = outputs.text_embeds if feature_type[
            0] == 'after' else outputs.text_model_output.pooler_output
    return feature
...
def fire_clip(text_encoder, meta, batch=1, max_objs=30, clip_model=None):
    ...
    out = {
        ...
        "text_masks":
            masks.unsqueeze(0).repeat(batch, 1) *
            complete_mask(meta["has_text_mask"], max_objs),
        "image_masks":
            masks.unsqueeze(0).repeat(batch, 1) *
            complete_mask(meta["has_image_mask"], max_objs),
        "text_embeddings":
...
def grounded_generation_box(loaded_model_list, instruction, *args, **kwargs):
    ...
    save_folder = os.path.join("create_samples",
                               instruction["save_folder_name"])
    ...
    if is_inpaint:
        ...
        input_image = (input_image.float().unsqueeze(0).to(device) / 255 -
                       0.5) / 0.5
        x0 = autoencoder.encode(input_image)
        if instruction["actual_mask"] is not None:
            inpainting_mask = instruction["actual_mask"][None, None].expand(
                batch['boxes'].shape[0], -1, -1, -1).to(device)
        else:
            # inpainting_mask = draw_masks_from_boxes( batch['boxes'], (x0.shape[-2], x0.shape[-1])  ).to(device)
            actual_boxes = [
                instruction['inpainting_boxes_nodrop']
                for _ in range(batch['boxes'].shape[0])
            ]
            inpainting_mask = draw_masks_from_boxes(
                actual_boxes, (x0.shape[-2], x0.shape[-1])).to(device)
    ...
    # ------------- prepare sampler ------------- #
    alpha_generator_func = partial(alpha_generator,
                                   type=instruction["alpha_type"])
    if False:
    ...
    samples_fake = sampler.sample(S=steps,
                                  ...
                                  input=input,
                                  uc=uc,
                                  guidance_scale=instruction['guidance_scale'],
