...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList([
            TimestepEmbedSequential(
                conv_nd(dims, total_in_channels, model_channels, 3, padding=1))
        ])
    ...
    def forward_position_net(self, input):
        if ("boxes" in input):
            boxes, masks, text_embeddings = input["boxes"], input[
                "masks"], input["text_embeddings"]
            ...
            text_embeddings = th.zeros(batch, self.max_box,
                                       self.positive_len).type(dtype).to(device)
    ...
    def forward_position_net_with_image(self, input):
        ...
        if ("boxes" in input):
            ...
            text_embeddings = th.zeros(batch, self.max_box,
                                       self.positive_len).type(dtype).to(device)
            image_embeddings = th.zeros(
                batch, self.max_box, self.positive_len).type(dtype).to(device)
        ...
        objs = self.position_net(boxes, masks, text_masks, image_masks,
                                 text_embeddings, image_embeddings)  # B*N*C
