...
class Evaluator:
    ...
    def __init__(self, config):
        ...
        # = = = = = create data = = = = = #
        self.dataset_eval = ConCatDataset(config.val_dataset_names,
                                          config.DATA_ROOT,
                                          config.which_embedder,
                                          train=False)
        print("total eval images: ", len(self.dataset_eval))
        sampler = DistributedSampler(
            self.dataset_eval, shuffle=False) if config.distributed else None
        loader_eval = DataLoader(self.dataset_eval,
        ...
        synchronize(
        )  # if rank0 is faster, it may mkdir before the other rank call os.listdir()
        if get_rank() == 0:
                ...
                if self.just_save_real:
                    ...
                    if self.config.inpaint:
                        image_mask = draw_masks_from_boxes(
                            batch['boxes'], self.model.image_size).to(device)
                        x0 = self.autoencoder.encode(batch["image"])

                    shape = (batch_size, self.model.in_channels,
