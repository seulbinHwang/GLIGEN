...
if device == "cpu":
    print(
        "It will be sloooow. Consider using GPU support with CUDA or (in case of M1/M2 Apple Silicon) MPS."
    )
elif device == "mps":
    print(
        "The fastest you can get on M1/2 Apple Silicon. Yet, still many opimizations are switched off and it will is much slower than CUDA."
    )
...
def parse_option():
    ...
    parser.add_argument("--folder",
                        ...
                        help="path to OUTPUT")
    parser.add_argument("--official_ckpt",
                        type=str,
                        default='ckpts/sd-v1-4.ckpt',
                        help="")
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="scale tanh(alpha). If 0, the behaviour is same as original model")
    parser.add_argument("--load-text-box-generation",
                        type=arg_bool,
                        default=True,
                        help="Load text-box generation pipeline.")
    parser.add_argument("--load-text-box-inpainting",
                        type=arg_bool,
                        default=False,
                        help="Load text-box inpainting pipeline.")
    parser.add_argument("--load-text-image-box-generation",
                        type=arg_bool,
                        default=False,
                        help="Load text-image-box generation pipeline.")
...
def load_ckpt_config_from_hf(modality):
    ...
    config = load_from_hf('gligen/demo_config_legacy',
                          filename=f'{modality}.pth')
    return ckpt, config


if args.load_text_box_generation:
    pretrained_ckpt_gligen, config = load_ckpt_config_from_hf(
...
if args.load_text_box_inpainting:
    pretrained_ckpt_gligen_inpaint, config = load_ckpt_config_from_hf(
        'gligen-inpainting-text-box')
    config = OmegaConf.create(config["_content"])  # config used in training
    config.update(vars(args))
    config.model['params']['is_inpaint'] = True
    config.model['params']['is_style'] = False
    loaded_model_list_inpaint = load_ckpt(config,
                                          pretrained_ckpt_gligen_inpaint)

if args.load_text_image_box_generation:
    pretrained_ckpt_gligen_style, config = load_ckpt_config_from_hf(
...
class Blocks(gr.Blocks):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(Blocks, self).__init__(theme, analytics_enabled, mode, title, css,
                                     **kwargs)
...
@torch.no_grad()
def inference(task, language_instruction, grounding_instruction,
              inpainting_boxes_nodrop, image, alpha_sample, guidance_scale,
              batch_size, fix_seed, rand_seed, actual_mask, style_image, *args,
              **kwargs):
    ...
    image_list = [placeholder_image] * len(
        phrase_list)  # placeholder input for visual prompt, which is disabled
    ...
    with torch.autocast(
            device_type='cuda',
            dtype=torch.float16) if device == "cuda" else nullcontext():
        if task == 'Grounded Generation':
            if style_image == None:
                return grounded_generation_box(loaded_model_list, instruction,
                                               *args, **kwargs)
            else:
                return grounded_generation_box(loaded_model_list_style,
                                               instruction, *args, **kwargs)
        elif task == 'Grounded Inpainting':
            assert image is not None
            instruction['input_image'] = image.convert("RGB")
            return grounded_generation_box(loaded_model_list_inpaint,
                                           instruction, *args, **kwargs)
...
def draw_box(boxes=[], texts=[], img=None):
    ...
    colors = [
        "red", "olive", "blue", "green", "orange", "brown", "cyan", "purple"
    ]
    ...
    for bid, box in enumerate(boxes):
        ...
        anno_text = texts[bid]
        draw.rectangle([
            box[0], box[3] - int(font.size * 1.2), box[0] + int(
                (len(anno_text) + 0.8) * font.size * 0.6), box[3]
        ],
                       outline=colors[bid % len(colors)],
                       fill=colors[bid % len(colors)],
                       width=4)
        draw.text(
            [box[0] + int(font.size * 0.2), box[3] - int(font.size * 1.2)],
            anno_text,
            font=font,
            fill=(255, 255, 255))
    return img
...
def get_concat(ims):
    ...
    dst = Image.new('RGB', (ims[0].width * n_col, ims[0].height * n_row),
                    color="white")
...
def generate(task, language_instruction, grounding_texts, sketch_pad,
             alpha_sample, guidance_scale, batch_size, fix_seed, rand_seed,
             use_actual_mask, append_grounding, style_cond_image, state):
    ...
    grounding_instruction = json.dumps({
        obj: box for obj, box in zip(grounding_texts, boxes)
    })
    ...
    if append_grounding:
        language_instruction = auto_append_grounding(language_instruction,
                                                     grounding_texts)
    ...
    for idx, gen_image in enumerate(gen_images):
        ...
        if task == 'Grounded Inpainting' and state.get('inpaint_hw', None):
            ...
            gen_image = sized_center_fill(state['original_image'].copy(),
                                          np.array(gen_image.resize((hw, hw))),
                                          hw, hw)
...
def draw(task, input, grounding_texts, new_image_trigger, state):
    ...
    if mask.shape != (512, 512):
        ...
        if 'original_image' in state and state[
                'original_image'].shape[:2] == mask.shape:
    ...
    if len(grounding_texts) < len(state['boxes']):
        grounding_texts += [
            f'Obj. {bid+1}'
            for bid in range(len(grounding_texts), len(state['boxes']))
        ]
    ...
    if box_image is not None and state.get('inpaint_hw', None):
        ...
        box_image = sized_center_fill(original_image, box_image_resize,
                                      inpaint_hw, inpaint_hw)
...
with Blocks(
    ...
    gr.Markdown(
        '<h1 style="text-align: center;">GLIGen: Open-Set Grounded Text-to-Image Generation</h1>'
    )
    ...
    with gr.Row():
        with gr.Column(scale=4):
            ...
            language_instruction = gr.Textbox(label="Language instruction",)
            grounding_instruction = gr.Textbox(
                label="Grounding instruction (Separated by semicolon)",)
            with gr.Row():
                sketch_pad = ImageMask(label="Sketch Pad",
            ...
            with gr.Accordion("Advanced Options", open=False):
                with gr.Column():
                    alpha_sample = gr.Slider(minimum=0,
                                             maximum=1.0,
                                             step=0.1,
                                             value=0.3,
                                             label="Scheduled Sampling (τ)")
                    guidance_scale = gr.Slider(minimum=0,
                                               maximum=50,
                                               step=0.5,
                                               value=7.5,
                    ...
                    append_grounding = gr.Checkbox(
                        value=True,
                        label="Append grounding instructions to the caption")
                    use_actual_mask = gr.Checkbox(
                        value=False,
                        label="Use actual mask for inpainting",
                        visible=False)
                    with gr.Row():
                        fix_seed = gr.Checkbox(value=True, label="Fixed seed")
                        rand_seed = gr.Slider(minimum=0,
                                              maximum=1000,
                                              step=1,
                                              value=0,
                                              label="Seed")
                    with gr.Row():
                        ...
                        style_cond_image = gr.Image(type="pil",
        ...
        with gr.Column(scale=4):
            ...
            with gr.Row():
                out_gen_3 = gr.Image(type="pil",
                                     visible=False,
                                     show_label=False)
                out_gen_4 = gr.Image(type="pil",
                                     visible=False,
                                     show_label=False)
        ...
        class Controller:
            ...
            def init_white(self, init_white_trigger):
                ...
                return np.ones((512, 512),
                               dtype='uint8') * 255, 1.0, init_white_trigger + 1
            ...
            def switch_task_hide_cond(self, task):
                ...
                return gr.Checkbox.update(
                    visible=cond, value=False), gr.Image.update(
                        value=None, visible=False), gr.Slider.update(
                            visible=cond), gr.Checkbox.update(
                                visible=(not cond), value=False)

        controller = Controller()
        main.load(lambda x: x + 1,
        ...
        sketch_pad.edit(
            draw,
            inputs=[
                task, sketch_pad, grounding_instruction,
                sketch_pad_resize_trigger, state
            ],
            outputs=[
                out_imagebox, sketch_pad_resize_trigger, image_scale, state
            ],
            queue=False,
        )
        ...
        clear_btn.click(clear,
                        ...
                        outputs=[
                            sketch_pad, sketch_pad_trigger, out_imagebox,
                            image_scale, out_gen_1, out_gen_2, out_gen_3,
                            out_gen_4, state
                        ],
                        queue=False)
        task.change(partial(clear, switch_task=True),
                    inputs=[task, sketch_pad_trigger, batch_size, state],
                    outputs=[
                        sketch_pad, sketch_pad_trigger, out_imagebox,
                        image_scale, out_gen_1, out_gen_2, out_gen_3, out_gen_4,
                        state
                    ],
                    queue=False)
        sketch_pad_trigger.change(
            ...
            queue=False)
        sketch_pad_resize_trigger.change(controller.resize_masked,
                                         inputs=[state],
                                         outputs=[sketch_pad, state],
                                         queue=False)
        batch_size.change(controller.change_n_samples,
                          inputs=[batch_size],
        ...
        gen_btn.click(
            ...
            queue=True)
        sketch_pad_resize_trigger.change(None,
                                         None,
                                         sketch_pad_resize_trigger,
                                         _js=rescale_js,
                                         queue=False)
        init_white_trigger.change(None,
                                  None,
                                  init_white_trigger,
                                  _js=rescale_js,
                                  queue=False)
        use_style_cond.change(lambda cond: gr.Image.update(visible=cond),
                              use_style_cond,
                              style_cond_image,
                              queue=False)
        task.change(controller.switch_task_hide_cond,
                    inputs=task,
                    outputs=[
                        use_style_cond, style_cond_image, alpha_sample,
                        use_actual_mask
                    ],
                    queue=False)
    ...
    with gr.Column():
        gr.Examples(
            ...
            inputs=[
                sketch_pad, task, language_instruction, grounding_instruction
            ],
            outputs=None,
            fn=None,
