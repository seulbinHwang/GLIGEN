...
class PositionNet(nn.Module):
    ...
    def __init__(self, resize_input=448, in_dim=152, out_dim=768):
        ...
        self.in_conv = nn.Conv2d(in_dim, 3, 3, 1,
                                 1)  # from num_sem to 3 channels
        ...
        self.pos_embedding = nn.Parameter(
            torch.empty(1, self.num_tokens,
                        convnext_feature_dim).normal_(std=0.02))  # from BERT
        ...
        self.null_feature = torch.nn.Parameter(
            torch.zeros([convnext_feature_dim]))

    def forward(self, sem, mask):
        B = sem.shape[0]

        # token from edge map
        sem = torch.nn.functional.interpolate(sem,
                                              self.resize_input,
                                              mode="nearest")
