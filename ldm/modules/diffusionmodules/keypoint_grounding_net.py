...
class PositionNet(nn.Module):
    ...
    def __init__(self, max_persons_per_image, out_dim, fourier_freqs=8):
        ...
        self.person_embeddings = torch.nn.Parameter(
            torch.zeros([max_persons_per_image, out_dim]))
        self.keypoint_embeddings = torch.nn.Parameter(torch.zeros([17,
                                                                   out_dim]))
        ...
        self.null_person_feature = torch.nn.Parameter(
            torch.zeros([self.out_dim]))
        self.null_xy_feature = torch.nn.Parameter(
    ...
    def forward(self, points, masks):
        ...
        person_embeddings = self.person_embeddings.unsqueeze(1).repeat(
            1, 17, 1).reshape(self.max_persons_per_image * 17, self.out_dim)
        keypoint_embeddings = torch.cat([self.keypoint_embeddings] *
                                        self.max_persons_per_image,
                                        dim=0)
        ...
        person_embeddings = person_embeddings * masks + (1 -
                                                         masks) * person_null
        xy_embedding = xy_embedding * masks + (1 - masks) * xy_null

        objs = self.linears(torch.cat([person_embeddings, xy_embedding],
                                      dim=-1))
