...
class CrossAttention(nn.Module):
    ...
    def fill_inf_from_mask(self, sim, mask):
        if mask is not None:
            B, M = mask.shape
            mask = mask.unsqueeze(1).repeat(1, self.heads,
                                            1).reshape(B * self.heads, 1, -1)
    ...
    def forward(self, x, key, value, mask=None):
        ...
        q = q.view(B, N, H, C).permute(0, 2, 1, 3).reshape(B * H, N,
                                                           C)  # (B*H)*N*C
        k = k.view(B, M, H, C).permute(0, 2, 1, 3).reshape(B * H, M,
                                                           C)  # (B*H)*M*C
        v = v.view(B, M, H, C).permute(0, 2, 1, 3).reshape(B * H, M,
                                                           C)  # (B*H)*M*C

        sim = torch.einsum('b i d, b j d -> b i j', q,
                           k) * self.scale  # (B*H)*N*M
        self.fill_inf_from_mask(sim, mask)
        attn = sim.softmax(dim=-1)  # (B*H)*N*M

        out = torch.einsum('b i j, b j d -> b i d', attn, v)  # (B*H)*N*C
        out = out.view(B, H, N, C).permute(0, 2, 1,
                                           3).reshape(B, N,
                                                      (H * C))  # B*N*(H*C)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.view(B, N, H, C).permute(0, 2, 1, 3).reshape(B * H, N,
                                                           C)  # (B*H)*N*C
        k = k.view(B, N, H, C).permute(0, 2, 1, 3).reshape(B * H, N,
                                                           C)  # (B*H)*N*C
        v = v.view(B, N, H, C).permute(0, 2, 1, 3).reshape(B * H, N,
                                                           C)  # (B*H)*N*C

        sim = torch.einsum('b i c, b j c -> b i j', q,
                           k) * self.scale  # (B*H)*N*N
...
class GatedCrossAttentionDense(nn.Module):
    ...
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head):
        ...
        self.attn = CrossAttention(query_dim=query_dim,
                                   key_dim=key_dim,
                                   value_dim=value_dim,
                                   heads=n_heads,
                                   dim_head=d_head)
    ...
    def forward(self, x, objs):
        ...
        x = x + self.scale * torch.tanh(self.alpha_dense) * self.ff(
            self.norm2(x))
...
class GatedSelfAttentionDense(nn.Module):
    ...
    def __init__(self, query_dim, context_dim, n_heads, d_head):
        ...
        self.attn = SelfAttention(query_dim=query_dim,
                                  heads=n_heads,
                                  dim_head=d_head)
        self.ff = FeedForward(query_dim, glu=True)
...
class GatedSelfAttentionDense2(nn.Module):
    ...
    def forward(self, x, objs):
        ...
        out = torch.nn.functional.interpolate(out, (size_v, size_v),
                                              mode='bicubic')
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_checkpoint=True):
        super().__init__()
        self.attn1 = SelfAttention(query_dim=query_dim,
                                   heads=n_heads,
                                   dim_head=d_head)
        self.ff = FeedForward(query_dim, glu=True)
        self.attn2 = CrossAttention(query_dim=query_dim,
                                    key_dim=key_dim,
                                    value_dim=value_dim,
                                    heads=n_heads,
                                    dim_head=d_head)
...
이러한 구조는 Attention 메커니즘을 통해 입력과 객체 특징을 결합하여 더 풍부한 표현을 학습할 수 있도록 합니다.
        ...
        if fuser_type == "gatedSA":
            ...
            # note key_dim here actually is context_dim
            self.fuser = GatedSelfAttentionDense2(query_dim, key_dim, n_heads,
                                                  d_head)
        elif fuser_type == "gatedCA":
            self.fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim,
                                                  n_heads, d_head)
        else:
            assert False
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlock(query_dim,
                                  key_dim,
                                  value_dim,
                                  n_heads,
                                  d_head,
